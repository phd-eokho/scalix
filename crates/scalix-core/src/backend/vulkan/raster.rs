...
pub const VERT_SPV: &[u32] = &[
    119734787, 65536, 524289, 50, 0, 131089, 1, 196622, 0, 1, 524303, 0, 35, 1852399981, 0, 12, 14,
    16, 262215, 12, 11, 42, 262215, 14, 30, 0, 196679, 10, 2, 327752, 10, 0, 11, 0, 131091, 1,
    196641, 2, 1, 262165, 3, 32, 0, 262165, 4, 32, 1, 196630, 5, 32, 262167, 6, 5, 2, 262167, 7, 5,
    4, 262187, 3, 8, 6, 262172, 9, 6, 8, 196638, 10, 7, 262176, 11, 1, 3, 262203, 11, 12, 1,
    262176, 13, 3, 6, 262203, 13, 14, 3, 262176, 15, 3, 10, 262203, 15, 16, 3, 262176, 17, 3, 7,
    262176, 32, 7, 9, 262176, 37, 7, 6, 262187, 4, 18, 0, 262187, 5, 19, 3212836864, 262187, 5, 20,
    1065353216, 262187, 5, 21, 0, 327724, 6, 22, 19, 19, 327724, 6, 23, 20, 19, 327724, 6, 24, 20,
    20, 327724, 6, 25, 19, 20, 327724, 6, 26, 21, 21, 327724, 6, 27, 20, 21, 327724, 6, 28, 21, 20,
    589868, 9, 29, 22, 23, 24, 24, 25, 22, 589868, 9, 30, 26, 27, 24, 24, 28, 26, 327734, 1, 35, 0,
    2, 131320, 36, 262203, 32, 33, 7, 262203, 32, 34, 7, 196670, 33, 29, 196670, 34, 30, 262205, 3,
    38, 12, 327745, 37, 39, 33, 38, 262205, 6, 40, 39, 327745, 37, 41, 34, 38, 262205, 6, 42, 41,
    196670, 14, 42, 327761, 5, 43, 40, 0, 327761, 5, 44, 40, 1, 458832, 7, 45, 43, 44, 21, 20,
    327745, 17, 46, 16, 18, 196670, 46, 45, 65789, 65592,
];
...
pub const FRAG_SPV: &[u32] = &[
    119734787, 65536, 524289, 20, 0, 131089, 1, 196622, 0, 1, 458767, 4, 15, 1852399981, 0, 12, 14,
    196624, 15, 7, 262215, 10, 34, 0, 262215, 10, 33, 0, 262215, 12, 30, 0, 262215, 14, 30, 0,
    131091, 2, 196641, 3, 2, 196630, 4, 32, 262167, 5, 4, 2, 262167, 6, 4, 4, 589849, 7, 4, 1, 0,
    0, 0, 1, 0, 196635, 8, 7, 262176, 9, 0, 8, 262203, 9, 10, 0, 262176, 11, 1, 5, 262203, 11, 12,
    1, 262176, 13, 3, 6, 262203, 13, 14, 3, 327734, 2, 15, 0, 3, 131320, 16, 262205, 8, 17, 10,
    262205, 5, 18, 12, 327767, 6, 19, 17, 18, 196670, 14, 19, 65789, 65592,
];
...
impl VulkanRasterResizer {
    ...
    pub fn new(ctx: Arc<VulkanContext>, profiler: Arc<dyn Profiler>) -> Self {
        ...
        let ring = Mutex::new(
            VulkanStagingRing::new(Arc::clone(&ctx), DEFAULT_RING_SLOTS)
                .expect("Failed to initialize Vulkan staging ring for rasterizer"),
        );
        Self {
            ctx,
            profiler,
    ...
    pub fn process(
        ...
        unsafe {
            ...
            // 2. Create Source Image and Destination Image
            ...
            let sampler = device.create_sampler(&sampler_info, None).map_err(|e| {
                ScalixError::ExecutionFailed(format!("Failed to create sampler: {e}"))
            })?;
            ...
            let pipeline_layout = device
                ...
                .map_err(|e| {
                    ScalixError::ExecutionFailed(format!("Failed to create pipeline layout: {e}"))
                })?;
            ...
            let render_pass = device
                ...
                .map_err(|e| {
                    ScalixError::ExecutionFailed(format!("Failed to create render pass: {e}"))
                })?;
            ...
            let vert_module = device
                ...
                .map_err(|e| {
                    ScalixError::ExecutionFailed(format!(
                        "Failed to create vert shader module: {e}"
                    ))
                })?;
            ...
            let frag_module = device
                ...
                .map_err(|e| {
                    ScalixError::ExecutionFailed(format!(
                        "Failed to create frag shader module: {e}"
                    ))
                })?;
            ...
            let framebuffer = device.create_framebuffer(&fb_info, None).map_err(|e| {
                ScalixError::ExecutionFailed(format!("Failed to create framebuffer: {e}"))
            })?;
            ...
            let desc_pool = device
                ...
                .map_err(|e| {
                    ScalixError::ExecutionFailed(format!("Failed to create descriptor pool: {e}"))
                })?;
            ...
            let desc_set = device
                ...
                .map_err(|e| {
                    ScalixError::ExecutionFailed(format!("Failed to allocate descriptor sets: {e}"))
                })?
                .into_iter()
                .next()
                .ok_or_else(|| {
                    ScalixError::ExecutionFailed("Allocated empty descriptor sets".to_string())
                })?;
            ...
            // 9. Command Buffer & Timestamps from slot
